...
export function ProductCard({
  ...
  return (
    <View className={`w-[48%] overflow-hidden rounded-3xl border shadow-sm ${dark ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
      <Pressable onPress={() => onOpen(product)}>
        <View className={`relative aspect-square overflow-hidden ${dark ? "bg-slate-800" : "bg-slate-100"}`}>
          {product.image_url ? (
            ...
            <View className="h-full items-center justify-center bg-orange-500">
              <View className="absolute left-4 top-4 h-14 w-20 rounded-xl bg-orange-200/70" />
              <View className="absolute bottom-5 right-5 h-16 w-16 rounded-full bg-amber-100/70" />
              <Text className="text-2xl font-black text-white">{String(product.name || "?").slice(0, 1).toUpperCase()}</Text>
            </View>
          )}
